...
export async function textToAudio(script: ScriptData): Promise<ScriptData> {
    ...
    const { destination, headers } = await downloadFile('http://0.0.0.0:8890/v0/generate_audio', getProjectPath(), {
        method: 'POST',
...
export async function generateBackingTrack(prompt: string, duration: number): Promise<BackingTrack> {
    ...
    const { destination } = await downloadFile('http://0.0.0.0:8896/v6/generate_audio', getProjectPath(), {
        method: 'POST',
